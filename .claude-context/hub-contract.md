...
The companion is a **thin client**. It never decides *what* to do — it reports what it sees/hears
...
3. Companion → `POST /v1/companion/pair`
   ...
   { "pairingCode": "7F3K-9Q2D",
     "device": { "platform": "ANDROID", "model": "vivo 1902", "osVersion": "9",
                 "appVersion": "1.0.0+1", "fcmToken": "<fcm registration token>" } }
   ```
...
```json
{ "index": 3, "label": "Search", "class": "android.widget.EditText",
  "cx": 360, "cy": 210, "clickable": true, "editable": true,
...
```json
{
  ...
  "agent": { "maxSteps": 15, "stepDelayMs": 1200, "sendScreenshots": "ON_REQUEST" },  // ALWAYS | ON_REQUEST | NEVER
  "heartbeatSec": 60,
