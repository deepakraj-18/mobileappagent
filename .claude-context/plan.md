...
> **Branch:** `feature/local-multi-device-mesh`
> **Status:** DRAFT v3 — rewritten 2026-09-05 after scope narrowed to "companion body only"
...
### Deferred / someday
- **Phase 7:** LAN mesh + PC console (`PROJECT_ANALYSIS.md`) — only worthwhile later as a *no-internet local mode*; not needed while the brain is cloud-hosted.
...
- **BD005 (presence) is Phase 5** even though "announce when near" sounds early — the *authoritative* signal is a hub command; local BLE is only the fallback, so it rides with the proactivity work.
...
| `flutter_background_service` *(maybe)* | Dart hook into the FGS | Evaluate vs a plain Kotlin service in SC004; prefer lighter. |
