...
| SC002 | Navigation shell — bottom nav `Operator · Settings`; onboarding gate; Dock as a separate full-screen route via the Companion-Mode toggle | SC | SC001 | 0.75 | In Progress |
...
| ID | Title | Stream | Dependencies | Est | Status |
|---|---|---|---|---|---|
| BD003 | `WakeWordService` — sherpa-onnx KWS, bundled model, duty-cycled start/stop with the FGS, wake-event stream; tap-to-talk fallback | BD | SC004 | 2.0 | Pending |
| BD004 | `VoiceSession` state machine (wake → STT → route: hub \| local-fallback \| command → TTS); voice `wake`/`sleep`. **+ `ScreenPowerPlugin.kt`** (sleep = `GLOBAL_ACTION_LOCK_SCREEN`/DevAdmin; wake = `FULL_WAKE_LOCK` + `KEEP_SCREEN_ON`) | BD | BD003, BD013 | 2.5 | Pending |
| FD001 | Dock screen shell — compact (360×772 dp): clock/date, listening state, wake/speaking animation, tap-to-talk, connection + presence chips (placeholders until Phase 3/5) | FD | SC002, BD013 | 1.5 | Pending |
