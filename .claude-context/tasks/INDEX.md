...
| BD004 | `VoiceSession` state machine (wake → STT → route: hub \| local-fallback \| command → TTS); voice `wake`/`sleep`. **+ `ScreenPowerPlugin.kt`** (sleep = `GLOBAL_ACTION_LOCK_SCREEN`/DevAdmin; wake = `FULL_WAKE_LOCK` + `KEEP_SCREEN_ON`) | BD     | BD003, BD013 | 2.5  | In Progress |
