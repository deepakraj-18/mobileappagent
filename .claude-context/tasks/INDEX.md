...
| BD041 | Local fallback LLM provider — single OpenAI-compatible endpoint+key (secure storage); `ai_service.dart` demoted to this; auto-engage when hub unreachable                            | BD     | BD040        | 1.0  | Review      |
| FD041 | Settings: fallback-provider form (base URL, key, model, test) + companion settings (wake phrase, "stay awake while charging", presence anchor picker, Vivo whitelist helper buttons) | FD     | SC002, BD041 | 1.25 | Review      |
| FI040 | Wire fallback provider + companion settings ↔ services; degraded-mode banner on Dock                                                                                                 | FI     | FD041, BD041 | 0.5  | In Progress |
